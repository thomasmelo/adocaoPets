...
<form action="{{
    ...
    <div class="col-md-2">
        ...
        <select id="id_especie" name="id_especie" class="form-select" required>
            ...
            <option value="{{ $item->id_especie}}" @selected( ( $pet && $pet->id_especie == $item->id_especie
                )
                ||
    ...
    <div class="col-md-3">
        ...
        <select id="id_sexo" name="id_sexo" class="form-select" required>
            ...
            <option value="{{$sexo->id_sexo}}" @selected( ( $pet && $pet->id_sexo == $sexo->id_sexo
                )
                ||
    ...
    <div class="col-md-12">
        ...
        <textarea class="form-control" name="observacao" id="observacao" cols="30" rows="5">{{
        $pet ?
        $pet->observacao :
        old('observacao')
        }}</textarea>
        ...
        success: function (response)
        {
            ...
            if( racas > 0 ){
                ...
                $.each(response, function(key, value){
                    options += '<option value="' + value.id_raca + '">' +value.raca+ '</option>';
                });
            }else{
